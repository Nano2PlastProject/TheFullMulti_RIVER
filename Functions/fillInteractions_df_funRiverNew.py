...
#    startTime = datetime.now()
    ...
    for sp in SpeciesList:
        RC_df_noadv = RC_df.drop("advection")
        RC_adv=RC_df[sp].advection
        if type(RC_df_noadv[sp]["fragmentation"]) is tuple:
            ...
            frag = RC_df_noadv[sp]["fragmentation"]
            #fragval = frag[0]*frag[1]

            RC_df_noFrag = RC_df_noadv[sp].drop("fragmentation")
            if isinstance(RC_adv, (int, float)):
                diag_list.append(-(sum(RC_df_noFrag)+frag[0]+RC_adv))
            else:
                diag_list.append(-(sum(RC_df_noFrag)+frag[0]+sum(RC_adv)))

        else:
            if isinstance(RC_adv, (int, float)):
                diag_list.append(-(sum(RC_df_noadv[sp])+RC_adv))
            else:
                diag_list.append(-(sum(RC_df_noadv[sp])+sum(RC_adv)))
    ...
    for sp1 in interactions_df.index:
       for sp2 in interactions_df.columns:
           ...
           if sp1 == sp2:
               ...
               #Processess inside the same ocean sector (RS)
               if sp1[:-3] == sp2[:-3]:
